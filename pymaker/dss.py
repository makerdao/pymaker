...
class DaiJoin(Join):
    ...
    def dai(self) -> DSToken:
        address = Address(self._contract.caller.dai())
...
class GemJoin(Join):
    ...
    def ilk(self):
        return Ilk.fromBytes(self._contract.caller.ilk())

    def gem(self) -> DSToken:
        address = Address(self._contract.caller.gem())
        return DSToken(self.web3, address)
...
class Vat(Contract):
    ...
    def live(self) -> bool:
        return self._contract.caller.live() > 0

    def wards(self, address: Address):
        assert isinstance(address, Address)

        return bool(self._contract.caller.wards(address.address))
    ...
    def can(self, sender: Address, usr: Address):
        ...
        return bool(self._contract.caller.can(sender.address, usr.address))
    ...
    def ilk(self, name: str) -> Ilk:
        ...
        (art, rate, spot, line, dust) = self._contract.caller.ilks(b32_ilk)
    ...
    def gem(self, ilk: Ilk, urn: Address) -> Wad:
        ...
        return Wad(self._contract.caller.gem(ilk.toBytes(), urn.address))

    def dai(self, urn: Address) -> Rad:
        assert isinstance(urn, Address)

        return Rad(self._contract.caller.dai(urn.address))

    def sin(self, urn: Address) -> Rad:
        assert isinstance(urn, Address)

        return Rad(self._contract.caller.sin(urn.address))

    def urn(self, ilk: Ilk, address: Address) -> Urn:
        assert isinstance(ilk, Ilk)
        assert isinstance(address, Address)

        (ink, art) = self._contract.caller.urns(ilk.toBytes(), address.address)
    ...
    def debt(self) -> Rad:
        return Rad(self._contract.caller.debt())

    def vice(self) -> Rad:
        return Rad(self._contract.caller.vice())

    def line(self) -> Rad:
        """ Total debt ceiling """
        return Rad(self._contract.caller.Line())
...
class Spotter(Contract):
    ...
    def vat(self) -> Address:
        return Address(self._contract.caller.vat())
...
class Vow(Contract):
    ...
    def __init__(self, web3: Web3, address: Address):
        ...
        self.address = address
        self._contract = self._get_contract(web3, self.abi, address)
        self.vat = Vat(web3, Address(self._contract.caller.vat()))
    ...
    def flapper(self) -> Address:
        return Address(self._contract.caller.flapper())

    def flopper(self) -> Address:
        return Address(self._contract.caller.flopper())

    def sin(self) -> Rad:
        return Rad(self._contract.caller.Sin())

    def sin_of(self, era: int) -> Rad:
        return Rad(self._contract.caller.sin(era))

    def ash(self) -> Rad:
        return Rad(self._contract.caller.Ash())

    def woe(self) -> Rad:
        return (self.vat.sin(self.address) - self.sin()) - self.ash()

    def wait(self) -> int:
        return int(self._contract.caller.wait())

    def sump(self) -> Rad:
        return Rad(self._contract.caller.sump())

    def bump(self) -> Rad:
        return Rad(self._contract.caller.bump())

    def hump(self) -> Rad:
        return Rad(self._contract.caller.hump())
...
class Jug(Contract):
    ...
    def __init__(self, web3: Web3, address: Address):
        ...
        self._contract = self._get_contract(web3, self.abi, address)
        self.vat = Vat(web3, Address(self._contract.caller.vat()))
        self.vow = Vow(web3, Address(self._contract.caller.vow()))
    ...
    def base(self) -> Wad:
        return Wad(self._contract.caller.base())

    def duty(self, ilk: Ilk) -> Ray:
        assert isinstance(ilk, Ilk)

        return Ray(self._contract.caller.ilks(ilk.toBytes())[0])

    def rho(self, ilk: Ilk) -> int:
        assert isinstance(ilk, Ilk)

        return Web3.toInt(self._contract.caller.ilks(ilk.toBytes())[1])
...
class Cat(Contract):
    ...
    def lump(self, ilk: Ilk) -> Wad:
        ...
        (flip, chop, lump) = self._contract.caller.ilks(ilk.toBytes())
        return Wad(lump)
